HEADER_METADATA_PROMPT = (
    ...
    "ALSO return these supporting line-index references (1-indexed integers, in "
    "document order, referencing the lines that comprise each field):\n"
    "- title_lines: list of int — the lines forming the title\n"
    "- abstract_lines: list of int — the lines forming the abstract\n"
    "- author_groups: list of list of int — one inner list per distinct author "
    "(e.g. [[3], [4], [5]] for three single-line authors)\n"
    "- keyword_groups: list of list of int — one inner list per distinct keyword\n"
    "Each index must reference an actual input line. Use empty lists when the "
    ...
    "the *_lines / *_groups lists — do not mix languages."
)
